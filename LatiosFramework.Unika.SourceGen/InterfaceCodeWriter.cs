        ...
        static void PrintPack(ref Printer printer, ref BodyContext context)
        {
                ...
                p.PrintEndLine(")");
                {
                    ...
                    if (hasReturn)
                    {
                        if (method.returnMod == Microsoft.CodeAnalysis.RefKind.None)
                            ...
                            printer.PrintLine("global::Latios.Unika.InternalSourceGen.StaticAPI.ContextPtr __ret = default;");
                    }
            ...
            for (int i = 0; i < context.properties.Count; i++)
            {
                ...
                if (property.hasGetter)
                {
                    ...
                    if (property.returnMod == Microsoft.CodeAnalysis.RefKind.None)
                        ...
                        printer.PrintLine("global::Latios.Unika.InternalSourceGen.StaticAPI.ContextPtr __ret = default;");
                    printer.PrintLine($"global::Latios.Unika.InternalSourceGen.StaticAPI.Dispatch(ref __data, {opId}, ref __ret);");
                    if (property.returnMod == Microsoft.CodeAnalysis.RefKind.None)
                        printer.PrintLine("return __ret;");
                    else
                    {
                ...
                if (property.hasSetter)
                {
                    ...
                    printer.PrintLine($"global::Latios.Unika.InternalSourceGen.StaticAPI.Dispatch(ref __data, {opId}, ref __propertyAssignArg);");
                    printer.CloseScope();
                    opId++;
                }
                printer.CloseScope();
            ...
            for (int i = 0; i < context.indexers.Count; i++)
            {
                ...
                if (indexer.hasGetter)
                {
                    ...
                    p = printer.PrintBeginLine($"global::Latios.Unika.InternalSourceGen.StaticAPI.Dispatch(ref __data, {opId}, ref __ret");
                    for (int j = 0; j < indexer.arguments.Count; j++)
                    {
